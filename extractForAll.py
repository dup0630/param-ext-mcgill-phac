...
# Get all subdirectories in the papers_directory. Each subdirectory is assumed to be a paper.
...
{pdf_text[:25000]}
"""
    print(f"\nProcessing standard extraction for paper {paper_id}...")
    try:
        ...
        print(f"Error processing paper {paper_id} (standard extraction): {e}")
...
df_raw = pd.DataFrame(raw_output_data, columns=["Papers", "Extracted Response", "overall CFR"])

# Define the expected columns for the standard extraction sheet.
standard_columns = [
    "PDF",
    "cases confirmed",
    "cases suspected",
...
# Ensure all expected columns exist: fill missing columns with empty strings.
for col in standard_columns:
    if col not in df_standard.columns:
        df_standard[col] = ""

# Calculate the 'calculated CFR' column from Numerator and Denominator.
df_standard["calculated CFR"] = df_standard.apply(
    lambda row: calculate_cfr(row.get("Numerator", ""), row.get("Denominator", "")),
    axis=1
)

# Reorder the DataFrame columns (the 'calculated CFR' column is appended at the end).
